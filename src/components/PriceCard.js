...
export default function PriceCard({
  ...
  const borderClass = highlighted ? "border-2 border-[#63899e]" : "border border-gray-200/60";
  const badgeClass =
    "absolute -top-3 left-1/2 -translate-x-1/2 bg-[#63899e] text-white text-xs px-3 py-1 rounded-full font-medium";

  return (
    <div className={`flex flex-col bg-white rounded-2xl shadow-sm p-8 relative ${borderClass}`}>
      ...
      <div className="min-h-[7rem]">
        ...
        <p
          className={
            priceSubline ? "text-sm text-green-600" : "text-sm text-transparent select-none"
          }
      ...
      <ul className="space-y-2 text-gray-600 mb-6 text-sm flex-1">
        {features.map(({ text, included }, i) => (
          <li key={i} className={included ? "" : "text-gray-400"}>
            {included ? "✓" : "✗"} {text}
          </li>
        ))}
      </ul>
